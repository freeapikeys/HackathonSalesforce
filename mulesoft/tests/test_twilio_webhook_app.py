...
class TwilioWebhookAppTest(unittest.TestCase):
    ...
    def test_mule_app_exposes_slack_interactivity_and_commands(self) -> None:
        ...
        self.assertIn('allowedMethods="POST"', config)
        self.assertIn("Parse Slack interaction", config)
        self.assertIn("googleOauthRequest", config)
        self.assertIn("gmailApiRequest", config)
        self.assertIn("/gmail/v1/users/me/messages/send", config)
        ...
        self.assertIn("Supplier email sent to", config)
        ...
        self.assertIn("safe manager cockpit", config)
        self.assertIn("Worker report captured", config)
        self.assertIn("Worker B", config)
