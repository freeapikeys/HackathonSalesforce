# Logia Implementation Plan

## Goal

Convert the existing HFS vertical slice into a Logia universal operations
...
- If the live Agentforce Studio preview drifts back to generic Salesforce
  ...
  `Logia_Hospital_Operations` authoring bundle on Windows.
...
This is the missing "front door" for the demo. Logia needs a credible path
...
## Workstream 5: Logia Command Center
...
- [x] Write expected recommendation notes for each scenario: what Logia
...
6. Freeze or protect `main` after the final merge owner confirms the branch,
   runs `npm run check`, and verifies the demo still tells one Logia story.
7. Rehearse the three-minute path from
   `docs/logia-demo-narrative.md`: trigger, evidence, Agentforce
