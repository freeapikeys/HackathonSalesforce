...
8. A source event arrives from an authorized business system or fixture.
9. The event contract validates the envelope, hash, idempotency key, sequence,
   and correlation ID.
10. The source payload maps into Salesforce context records for entities,
    relationships, evidence, work items, recommendations, approvals, actions, and
    outcomes.
11. Agentforce receives only the context the current user and purpose can access.
12. Specialist agents analyze the relevant relationship, operational, financial,
    service, supplier, staff, or customer context.
13. The orchestrator produces one evidence-backed recommendation or plan.
14. The responsible human approves, rejects, modifies, or defers protected
    actions.
15. MuleSoft mocks execute approved write-backs and channel alerts.
16. Outcome events return through the same intake path and refresh the command
